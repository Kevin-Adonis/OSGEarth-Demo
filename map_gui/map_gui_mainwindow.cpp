    ...
    ui(new Ui::MAP_GUI_MainWindow)
{
    ...
    //中间地图显示
    m_central3DMapCanvas = new MAP_GUI_DroppableCanvas();
    //    m_osgWidget = MAP_OSG_ViewController::addView(
    //                      "test.earth");
    m_osgWidget = MAP_OSG_ViewController::addView(
                      qApp->applicationDirPath() + QString("/example_data/base.earth"));
    MAP_GLOBALDATA_APPData::getInstance()->g_osgWidget = m_osgWidget;
    ...
    m_templateTree = new MAP_GUI_TestTreeView(m_templateDock);
    ...
    connect(ui->m_createPointPtn, &QToolButton::clicked,
            this, &MAP_GUI_MainWindow::drawingPrimitivePtnClicked);
    connect(ui->m_createLinePtn, &QToolButton::clicked,
            this, &MAP_GUI_MainWindow::drawingPrimitivePtnClicked);
...
void MAP_GUI_MainWindow::drawingPrimitivePtnClicked()
{
    auto l_sender = dynamic_cast<QToolButton*>(sender());
    if(!l_sender)
        return;
    QString l_objectName = l_sender->objectName();
    if(l_objectName == "m_createPointPtn")
        MAP_GLOBALDATA_APPData::getInstance()->g_primitiveMode = MAP_GLOBALDATA_APPData::POINT;
    else if(l_objectName == "m_createLinePtn")
        MAP_GLOBALDATA_APPData::getInstance()->g_primitiveMode = MAP_GLOBALDATA_APPData::LINE;
    MAP_GLOBALDATA_Controller::mainWindowController->drawingPrimitive();
}

void MAP_GUI_MainWindow::closeEvent(QCloseEvent *event)
{
    qApp->quit();
}

void MAP_GUI_MainWindow::on_m_latlongGridPtn_clicked()
{
    MAP_GLOBALDATA_Controller::mainWindowController->showLatLongGrid();
}
void MAP_GUI_MainWindow::on_m_conventionPtn_clicked()
{
    auto l_mapData = MAP_GLOBALDATA_APPData::getInstance()->g_mapData;
    if(!l_mapData->valid)
        return;
    auto l_earthManipulator = l_mapData->pEarthManipulator;
    auto l_settings = l_earthManipulator->getSettings();
    l_settings->bindMouse(osgEarth::Util::EarthManipulator::ACTION_NULL,
                          osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON);
    l_earthManipulator->applySettings(l_settings);
}

void MAP_GUI_MainWindow::on_m_romaingPtn_clicked()
{
    auto l_mapData = MAP_GLOBALDATA_APPData::getInstance()->g_mapData;
    if(!l_mapData->valid)
        return;
    ...
    auto l_earthManipulator = l_mapData->pEarthManipulator;
    auto l_settings = l_earthManipulator->getSettings();
    l_settings->bindMouse(osgEarth::Util::EarthManipulator::ACTION_PAN,
                          osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON);
    l_earthManipulator->applySettings(l_settings);
}

void MAP_GUI_MainWindow::on_m_magnifyEarthPtn_clicked()
{
    //放大
    auto l_mapData = MAP_GLOBALDATA_APPData::getInstance()->g_mapData;
    if(!l_mapData->valid)
    ...
    auto l_earthManipulator = l_mapData->pEarthManipulator;
    if(l_earthManipulator->getViewpoint().getRange() > 10000)
        l_earthManipulator->zoom(0, -0.2);
}

void MAP_GUI_MainWindow::on_m_reduceEarthPtn_clicked()
{
    //缩小
    auto l_mapData = MAP_GLOBALDATA_APPData::getInstance()->g_mapData;
    if(!l_mapData->valid)
...
void MAP_GUI_MainWindow::on_m_DistancePtn_clicked()
{
    DISCLASS::disObject->MeasuringDistance();

}

void MAP_GUI_MainWindow::on_m_directionPth_clicked()
{
    MAP_GLOBALDATA_Controller::mainWindowController->showCompass(m_osgWidget->width(), m_osgWidget->height());
}
void MAP_GUI_MainWindow::on_m_layerControllerPtn_clicked()
{
    if(m_layerControlDock->isVisible())
        m_layerControlDock->setVisible(false);
    else
        m_layerControlDock->setVisible(true);
}
